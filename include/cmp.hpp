...
public:
	...
	template<typename T>
	inline T* real_pointer(void* pointer) const { return reinterpret_cast<T*>((uintptr_t)base + (uintptr_t)pointer); }

	template<typename T>
	inline void* pool_pointer(T* pointer) const { return (void*)((uintptr_t)pointer - (uintptr_t)base); }
